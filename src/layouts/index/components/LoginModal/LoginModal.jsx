...
export default function LoginModal() {
  ...
  useEffect(() => {
    Ajax({ url: 'User_Login' }, { uname: '童晨'}).then(res => {
      if (res.code) {
        message.success(res.message, 500)
      }
    })
  });
      ...
      <Tabs size="large">
        ...
        <TabPane tab={<i className="iconfont">&#xe605;</i>} key="2">
          ...
          >
            ...
            <Form.Item name="email" rules={[{ required: true, message: '请输入正确格式的邮箱', pattern: /^[a-zA-Z0-9]+([-_.][a-zA-Z0-9]+)*@[a-zA-Z0-9]+([-_.][a-zA-Z0-9]+)*\.[a-z]{2,}$/ }]} label="邮箱">
              <Input prefix={<RiseOutlined />} placeholder="Email" />
            </Form.Item>

            <Form.Item name="phone" rules={[{ required: true, message: '请输入正确格式的电话号码', pattern: /^1(2|3|4|5|7|8)\d{9}$/ }]} label="电话">
